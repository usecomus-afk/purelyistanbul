        ...
        <div className="flex items-center gap-4 overflow-x-auto py-3">
          {MARKETPLACE_CATEGORIES.map((c) => (
            <button key={c.key} onClick={() => handleSelect(c.key)} className="flex flex-col items-center gap-1.5 shrink-0">
              <span
                className={`w-12 h-12 rounded-full flex items-center justify-center border-2 p-2.5 transition-colors ${
                  value === c.key ? "bg-ink border-ink" : "bg-white border-sand-border"
                }`}
              >
                <span className="relative w-full h-full">
                  <Image
                    ...
                    alt=""
                    fill
                    className={`object-contain ${value === c.key ? "brightness-0 invert" : ""}`}
                  />
                </span>
              </span>
